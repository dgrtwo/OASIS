...
class AnnotatedGenome:
    ...
    def __init__(self, genome_file=None, lst=None, annotated=False,
        ...
        if genome_file:
            ## CHANGE: do not parse with genbankparse, instead use simple parser
        ...
        if data_folder == None:
            data_folder = "data"
        nucfile = os.path.join(data_folder, ISFINDER_NUCLEOTIDE_FILE)
        aafile = os.path.join(data_folder, ISFINDER_AA_FILE)
        self.profile = Profile.Profile(nucfile, aafile)
...
#                             print self.candidates[j]
    ...
    def print_relevant(self, name):
        """test function"""
        
        print "Point " + str(name) + ":"
        for i, s in enumerate(self.annotations):
            st = str(s)
            if "y1853/y1854" in st:
                print "annotation", i
                print st
    ...
    def write_txt(self, outfile):
        """write annotations as a tab-delimited text file"""
        outf = open(outfile + ".txt", "w")
        for is_set in self.annotations:
            outf.write(str(is_set))
        outf.close()
    ...
    def __find_partials(self, minimum_blast_length=0):
        ...
        blast_db = "OASIS_temp_genome.fasta"
        ...
        blast_file = "OASIS_temp_IS.fasta"
        ...
        os.system("rm " + blast_db)
        os.system("rm " + blast_file)
        os.system("rm " + blast_db + ".n*")
    ...
    def get_aaseq(self, f):
        ...
        if (end - start) % 3 != 0:
            raise Exception("aaseq of " + str(f) + "is wrong length!")
    ...
    def find_IRs_around(self, c=None, lst=None):
        ...
        if dir == 1:
            ...
            #print chromosome[start-500: end+500]
        else:
